...
test_that("sig_eval_table requires predicted split names to exist in split table", {
  source_sigs <- list(drugA = c("g1", "g2"))
  pred_sigs <- list(wrong_split = list(drugA = c("g1", "g2")))
  ...
  expect_error(
    ...
    "All names\\(pred_sigs\\) must be present as split columns in 'split_file' when 'splits = TRUE'\\. Missing columns: wrong_split"
  )
})

test_that("sig_eval_table evaluates only splits present in pred_sigs", {
  source_sigs <- list(
    drugA = c("g1", "g2"),
    drugB = c("g2", "g3"),
    drugC = c("g3", "g4")
  ...
  expect_equal(unique(eval_df$split), 2L)
  expect_equal(eval_df$gene, "drugB")
})
