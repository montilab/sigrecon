---
title: "Benchmarking Design: Splits and Evaluation Metrics"
...
ctrl/10th/90th split design used to benchmark methods at varying amounts of
...
A recontextualization method that's simply handed the true target-context
signature as extra input isn't really being tested -- it's cheating. The
package's benchmarking study evaluates methods under three conditions that
control how much target-context information is available:

- **`ctrl`**: none of the target perturbations' information is available to
  the method beyond the source signature itself (what every example so far
  in this vignette series has done).
- **`ctrl_10th`**: the method is allowed 1/10th of the target perturbations
  as reference (e.g. to tune a network or model), then evaluated on the
  *other* 9/10th -- the harder, held-out majority.
- **`ctrl_90th`**: the reverse -- allowed 9/10th, evaluated on the remaining
  1/10th.
...
column per split (`split_1`, `split_2`, ...), each a logical vector marking
a fixed ~10%/90% partition of perturbations, aligned to a
...
perturbations get evaluated for you.
...
- The [Network Propagation in Depth](network-propagation-details.html)
  vignette for `network_sig()`'s options
- `?sig_eval_table`, `?paired_eval_table` for full parameter references
