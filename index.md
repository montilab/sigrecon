...
the benchmarking study (Perturb-seq, SciPlex, DrugMatrix, Tahoe); the
full pseudobulk expression and perturbational signatures for each are
archived on Zenodo (see [Data](#data) below).
...
Full-size pseudobulk expression and perturbational signatures for each
dataset in the benchmarking study are archived on Zenodo:
...
The small, bundled `demo_*` datasets used above are subsets of these;
use
[`get_dataset()`](https://montilab.github.io/sigrecon/reference/get_dataset.md)/[`list_datasets()`](https://montilab.github.io/sigrecon/reference/list_datasets.md)
for the full processed signature sets, or the Zenodo records above for
the underlying pseudobulk expression.
